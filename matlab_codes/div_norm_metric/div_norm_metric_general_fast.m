function [d_ef,d_fe,d2,spatial_dist_map,r1,r2,ind]=div_norm_metric_general(r1,im2);

% DIV_NORM_METRIC_GENERAL_FAST computes the perceptual difference between 2 images
...
% optimal parameters) for distorted image (it assumes the transform known for the original) 
% and applies the optimal summation model to the difference vector.
%
...
%
% USE: [d_ef,d_fe,d2,map,r1,r2,ind]=div_norm_metric_general_fast(r1,im2);
...
d_fe=1000*d_fe*numel(delta_r(1:end-prod(ind(end,:)),:))/(3*numel(im2(:,:,1)));
d_ef=1000*d_ef*numel(delta_r(1:end-prod(ind(end,:)),:))/(3*numel(im2(:,:,1)));
d2=1000*d2*numel(delta_r(1:end-prod(ind(end,:)),:))/(3*numel(im2(:,:,1)));